...
int searchmrupage(struct process_info PT[],int frames[],int n){
	int max=INT_MIN;
	int index;
	for(int i=0;i<n;i++){
		if(PT[frames[i]].last_access_time > max){
			max=PT[frames[i]].last_access_time;
			index=i;
		}
	}
	return index;
	...
	for(int i=0;i<n;i++){
		if(!(ispagepresent(PT,reference_string[i]))){
			...
			if(flag==false && current<no_of_frames){
				...
				int MRU_index;
				MRU_index=searchmrupage(PT,frames,no_of_frames);
				updatepagetable(PT,frames[MRU_index],-1,0,i);
				frames[MRU_index]=reference_string[i];
				printframe(frames,no_of_frames);
				updatepagetable(PT,reference_string[i],MRU_index,1,i);
			}
		}
